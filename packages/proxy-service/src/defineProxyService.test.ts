import { defineProxyService, defineShakableProxyService } from './defineProxyService';
...
const defineShakableTestService = () =>
  defineShakableProxyService<ShakableTestService>('ShakableTestService');
...
describe('defineShakableProxyService', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    fakeBrowser.reset();
  });
  ...
  it('should support executing functions directly', async () => {
    const expected = 5;
    const fn: () => Promise<void> = vi.fn().mockResolvedValue(expected);
    const [registerFn, getFn] = defineShakableProxyService<typeof fn>('fn');
    registerFn(() => fn);
  ...
  it('should support executing deeply nested functions at multiple depths', async () => {
    ...
    const [registerDeepObject, getDeepObject] =
      defineShakableProxyService<typeof deepObj>('DeepObject');
